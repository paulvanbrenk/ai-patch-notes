...
public class SyncServiceTests : IDisposable
{
    ...
    public async Task SyncPackageAsync_WithNewReleases_AddsToDatabase()
    {
        ...
        releases.Should().Contain(r => r.Version == "v1.0.0");
        releases.Should().Contain(r => r.Version == "v1.1.0");
    }

    [Fact]
    public async Task SyncPackageAsync_ParsesVersionFields()
    {
        // Arrange
        var package = new Package { Name = "pkg", Url = "https://github.com/owner/repo", NpmName = "pkg", GithubOwner = "owner", GithubRepo = "repo" };
        _db.Packages.Add(package);
        ...
        SetupGitHubReleases("owner", "repo", [
            CreateRelease("v2.3.0", DateTime.UtcNow),
            CreateRelease("v2.4.0-beta.1", DateTime.UtcNow)
        ]);
        ...
        // Assert
        var releases = await _db.Releases.Where(r => r.PackageId == package.Id).OrderBy(r => r.Version).ToListAsync();
        var stable = releases.First(r => r.Version == "v2.3.0");
        stable.Major.Should().Be(2);
        stable.Minor.Should().Be(3);
        stable.IsPrerelease.Should().BeFalse();

        var prerelease = releases.First(r => r.Version == "v2.4.0-beta.1");
        prerelease.Major.Should().Be(2);
        prerelease.Minor.Should().Be(4);
    ...
    public async Task SyncPackageAsync_SkipsDraftReleases()
    {
        ...
        var releases = await _db.Releases.Where(r => r.PackageId == package.Id).ToListAsync();
        releases.Should().ContainSingle(r => r.Version == "v1.1.0");
    }
    ...
    [Fact]
    public async Task SyncPackageAsync_SkipsDuplicateVersions()
    {
        // Arrange
        var package = new Package { Name = "pkg", Url = "https://github.com/owner/repo", NpmName = "pkg", GithubOwner = "owner", GithubRepo = "repo" };
        _db.Packages.Add(package);
    ...
    public async Task SyncPackageAsync_StopsAtOlderReleases_WhenLastFetchedAtIsSet()
    {
        ...
        var releases = await _db.Releases.Where(r => r.PackageId == package.Id).ToListAsync();
        releases.Should().ContainSingle(r => r.Version == "v1.2.0");
    }
