...
public class GitHubClientTests : IDisposable
{
    ...
    public async Task GetNotificationsAsync_ReturnsNotifications()
    {
        ...
        var notifications = new[]
        {
            new
            {
                ...
                subject = new { title = "Issue 1", type = "Issue" },
                repository = new { id = 123L, name = "repo", full_name = "owner/repo", owner = new { login = "owner", id = 1L } }
            }
        };
        _mockHandler.SetupResponse("notifications?per_page=50&page=1", notifications);

        // Act
        var result = await _client.GetNotificationsAsync();

        // Assert
        result.Should().ContainSingle();
        result[0].Id.Should().Be("1");
        result[0].Reason.Should().Be("mention");
        result[0].Repository.FullName.Should().Be("owner/repo");
    }

    [Fact]
    public async Task GetNotificationsAsync_WithAllParameter_IncludesInQuery()
    {
        // Arrange
        _mockHandler.SetupResponse("notifications?per_page=50&page=1&all=true", Array.Empty<object>());

        // Act
        await _client.GetNotificationsAsync(all: true);

        // Assert
        _mockHandler.LastRequestUri.Should().Contain("all=true");
    }

    [Fact]
    public async Task GetNotificationsAsync_WithSinceParameter_IncludesInQuery()
    {
        // Arrange
        var since = new DateTime(2025, 1, 15, 12, 0, 0, DateTimeKind.Utc);
        _mockHandler.SetupResponse("notifications?per_page=50&page=1&since=2025-01-15T12:00:00.0000000Z", Array.Empty<object>());

        // Act
        await _client.GetNotificationsAsync(since: since);

        // Assert
        _mockHandler.LastRequestUri.Should().Contain("since=");
    }
    ...
    [Fact]
    public async Task GetAllNotificationsAsync_WithMultiplePages_FetchesAll()
    {
        // Arrange
        var page1 = Enumerable.Range(1, 50)
            .Select(i => new
            {
                id = i.ToString(),
                unread = true,
                reason = "mention",
                updated_at = DateTime.UtcNow,
                subject = new { title = $"Issue {i}", type = "Issue" },
                repository = new { id = (long)i, name = "repo", full_name = "owner/repo", owner = new { login = "owner", id = 1L } }
            })
            .ToArray();
        var page2 = Enumerable.Range(51, 25)
            .Select(i => new
            {
                id = i.ToString(),
                unread = true,
        ...
        _mockHandler.SetupResponse("notifications?per_page=50&page=1", page1);
        _mockHandler.SetupResponse("notifications?per_page=50&page=2", page2);

        // Act
        var result = new List<GitHubNotification>();
        await foreach (var notification in _client.GetAllNotificationsAsync())
        {
